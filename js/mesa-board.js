...
// ---- Seleção múltipla de tokens (ferramenta 🔲, boardTool === 'select') --
// Conjunto de ids marcados: com a ferramenta ativa, clicar em cada token
// marca/desmarca; arrastar qualquer um dos marcados move o grupo inteiro
// junto (ver attachTokenDragHandlers, em mesa-tokens.js), preservando a
// formação relativa entre eles. Independente de selectedTokenId (que só
// serve pras alças de girar/redimensionar de um único token, na ferramenta
// "Mover" padrão).
...
function closeTable() {
  ...
  // Reseta o controle de "rolagem já vista" — senão, ao trocar de mesa, o
  // histórico da mesa nova (ids que nunca passaram por diceRollSeenIds
  // nesta sessão) seria tratado como "primeiro carregamento" só por sorte,
  // mas os toasts órfãos da mesa anterior e o estado de "visto" ficariam
  // misturados entre mesas diferentes.
